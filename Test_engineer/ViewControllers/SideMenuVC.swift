...
extension SideMenuViewController{    
    private func setupFullData() ->[ExpandingCellModel] {
        ...
        array.append(ExpandingCellModel(label: "Blurs", operations: [
            OperationCellModel(label: "Blur", operation: .presentController(.KERNEL_VC(.BLUR))),
            OperationCellModel(label: "Gaussian Blur", operation: .presentController(.KERNEL_VC(.GAUSSIAN))),
            OperationCellModel(label: "Sobel blur", operation: .presentController(.KERNEL_VC(.SOBEL))),
            OperationCellModel(label: "Laplasian blur", operation: .presentController(.KERNEL_VC(.LAPLASSIAN))),
            OperationCellModel(label: "Canny blur", operation: .presentController(.KERNEL_VC(.CANNY)))]
        ))
        
        array.append(ExpandingCellModel(label: "Bitwise Operation", operations: [
            OperationCellModel(label: "AND", operation: .presentController(.BITWISE_VC(.AND))),
            OperationCellModel(label: "OR", operation: .presentController(.BITWISE_VC(.OR))),
            OperationCellModel(label: "NOT", operation: .presentController(.BITWISE_VC(.NOR))),
            OperationCellModel(label: "XOR", operation: .presentController(.BITWISE_VC(.XOR)))]
        ))
        
        array.append(ExpandingCellModel(label: "Morphology Operation", operations: [
            OperationCellModel(label: "Erode", operation: .modifyImage(.ERODE)),
            OperationCellModel(label: "Dilate", operation: .modifyImage(.DILATE)),
            OperationCellModel(label: "Open", operation: .modifyImage(.OPEN)),
            OperationCellModel(label: "Close", operation: .modifyImage(.CLOSE)),
            OperationCellModel(label: "Skeletonize", operation: .modifyImage(.SKELETONIZE)),
            OperationCellModel(label: "Watershed", operation: .modifyImage(.WATERSHED))]
        ))
